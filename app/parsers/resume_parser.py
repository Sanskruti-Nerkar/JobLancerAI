...
class ResumeParser:
    ...
    def _is_valid_skill(self, skill: str) -> bool:
        ...
        # Check partial matches with known skills
        for known_skill in KNOWN_TECHNICAL_SKILLS:
            if (len(skill) > 3 and len(known_skill) > 3):
                if skill in known_skill or known_skill in skill:
                    return True
        ...
        # Reject if it's all digits or contains only common words
        if skill.isdigit():
            return False
        ...
        # Check if it looks like a technical term (has technical patterns)
        technical_patterns = [
            r'[a-z]+\+\+',  # C++
            r'[a-z]+#',     # C#
    ...
    def _extract_skills(self, text: str) -> Dict[str, List[str]]:
        ...
        # 1. Extract known skills directly
        ...
        # 2. Look for skills in bullet points and lists
        lines = text.split('\n')
        for line in lines:
            line = line.strip().lower()
            
            # Skip section headers
            if any(header in line for header in ['skills:', 'technical skills:', 'tools:', 'technologies:']):
                continue
            
            # Extract from bullet points
            if line.startswith(('•', '-', '*', '·', '→')):
                # Remove bullet and split by comma
                line = re.sub(r'^[•\-\*·→]\s*', '', line)
                parts = re.split(r'[,;|]', line)
                
                for part in parts:
                    part = part.strip()
                    if self._is_valid_skill(part):
                        # Normalize using aliases
                        normalized = self._normalize_skill(part)
                        technical_skills.add(normalized)
        
        # 3. Look for patterns like "proficient in X"
        skill_patterns = [
            r'(?:proficient in|experience with|knowledge of|skilled in|expertise in|working with)\s+([a-z0-9\s\.\+#\-]+)',
            r'(?:using|used)\s+([a-z0-9\s\.\+#\-]+)',
        ]
        ...
        for pattern in skill_patterns:
            ...
            for match in matches:
                ...
                skill_candidates = re.split(r'[,;&]|\band\b|\bor\b', skills_str)
        ...
        print(f"\n✅ Extracted {len(cleaned_skills)} technical skills")
        print(f"Skills: {cleaned_skills[:20]}")
        if len(cleaned_skills) > 20:
            print(f"... and {len(cleaned_skills) - 20} more")
        
        return {
            'technical': cleaned_skills,
            'soft': []  # Focus on technical skills for job matching
        }
