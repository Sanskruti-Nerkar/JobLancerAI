...
class JobMatcher:
    ...
    def __init__(self):
        ...
        # Caches to avoid recomputation across multiple resumes
        self._jobs_cache_key = None
        self._job_texts_cache: List[str] = []
        self._job_skills_cache: List[List[str]] = []

    def _compute_jobs_cache_key(self, jobs: List[Dict]) -> str:
        """Compute a lightweight cache key for a set of jobs."""
        # Use a tuple of (len, first 5 titles+links) to detect changes quickly
        preview = tuple(
            (
                (job.get('title') or ''),
                (job.get('company') or ''),
                (job.get('link') or ''),
                len(job.get('summary', '') or '')
            )
            for job in jobs[:5]
        )
        tail = tuple(
        ...
        return f"{len(jobs)}|{hash(preview)}|{hash(tail)}"
    ...
    def fit_transform(self, jobs: List[Dict], resume_text: str) -> None:
        ...
        """
        cache_key = self._compute_jobs_cache_key(jobs)

        # If jobs unchanged, reuse vectors and only transform resume
        if self._jobs_cache_key == cache_key and self.job_vectors is not None:
            resume_processed = self.preprocess_text(resume_text)
            self.resume_vector = self.vectorizer.transform([resume_processed])
            return

        # Preprocess all texts and precompute skills for each job
        job_descriptions: List[str] = []
        job_skills_cache: List[List[str]] = []
        for job in jobs:
            ...
            # Prefer explicit key_skills; fallback to extraction
            if key_skills:
                job_skills_cache.append([s.lower().strip() for s in key_skills if isinstance(s, str)])
            else:
                job_skills_cache.append(self.extract_skills(job_text))
        ...
        self._job_texts_cache = job_descriptions
        self._job_skills_cache = job_skills_cache

        # Transform resume
        self.resume_vector = self.vectorizer.transform([resume_processed])
    
    def calculate_similarity_scores(self, jobs: List[Dict], resume_text: str, resume_skills_external: List[str] = None) -> List[Dict]:
        """
        ...
        if resume_skills_external is not None:
            # Use parsed resume skills if provided
        ...
        for i, job in enumerate(jobs):
            ...
            # Skills match with more detailed analysis - use precomputed cache
            if 0 <= i < len(self._job_skills_cache):
                job_skills = self._job_skills_cache[i]
            else:
    ...
    def rank_jobs(self, jobs: List[Dict], resume_text: str, 
        ...
        """
        results = self.calculate_similarity_scores(jobs, resume_text, resume_skills_external=resume_skills)
    ...
    def get_top_matches(self, jobs: List[Dict], resume_text: str, 
                       top_n: int = 10, min_score: float = 0.3, resume_skills: List[str] = None) -> List[Dict]:
        """
